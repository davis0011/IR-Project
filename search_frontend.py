...
def search():
    ...
    global body_index, meta_dict, d_avgm, title_index, model, anchor_index
    ...
    c_title = Counter()
    for word in main_array:
        if word not in title_index.posting_locs:
            continue
        res = myread(title_index, word)
        for post in res:
            c_title[post[0]] += 1
    title_max = c_title.most_common(1)[0][1]
    """
    Combination of BM25, title  and anchor weights with normalization
    """
    for key in c_title:
        c_title[key] = c_title[key] / title_max * 2

    c_anchor = Counter()
    for word in main_array:
        if word not in anchor_index.posting_locs:
            continue
        res = myread(anchor_index, word)
        for post in res:
            c_anchor[post[0]] += 1
    for key in c_anchor:
        c_anchor[key] = c_anchor[key] / title_max/2
    c_final = c_body + c_title + c_anchor
    ...
    print(c_final.most_common(3))
    ret = [(int(x[0]), meta_dict[x[0]][0]) for x in c_final.most_common(50) if x[0]]
...
def search_body():
    ...
    N = len(meta_dict)
    for word in main_array:
        if word not in body_index.posting_locs:
            continue
        i = main_array.index(word)
        res = myread(title_index, word)
        for post in res:
    ...
    ret = [(x[0], meta_dict[x[0]][0]) for x in final_countdown.most_common(100) if x[0]]
    # END SOLUTION
