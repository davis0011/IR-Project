...
class MultiFileReader:
    ...
    def read(self, locs, n_bytes,index):
        b = []
        if type(locs) != list:
            locs = [locs]
        # return
        for f_name, offset in locs:
            ...
            if f_name not in self._open_files:
                # ./home/shavitda/{index}/{f_name}
                self._open_files[f_name] = open(index+"/"+f_name, 'rb')
    ...
    def posting_lists_iter(self):
        ...
        with closing(MultiFileReader()) as reader:
            for w, locs in self.posting_locs.items():
                b = reader.read(locs[0], self.df[w] * TUPLE_SIZE,self.name)
                posting_list = []
